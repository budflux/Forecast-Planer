...
const offsetReconciles = results.slice(1).every((row, index) => Math.abs(Number(row.offsetBalance || 0) - Math.max(0, Number(results[index].offsetBalance || 0) + Number(row.weeklySurplus || 0))) < 0.01);
document.getElementById('report-summary').textContent = `${results.length} weekly forecast results — offset check ${offsetReconciles ? 'passed' : 'failed'}`;
