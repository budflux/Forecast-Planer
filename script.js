...
class CostProjectorApp {
  ...
  async start() {
    ...
    this.load(); this.bindEvents(); this.render(); this.showPage(localStorage.getItem('activePage') || 'page-forecast');
  }
  showAuth() {
  ...
  showPage(page) { if (!document.getElementById(page)?.classList.contains('app-page')) page = 'page-forecast'; localStorage.setItem('activePage', page); document.querySelectorAll('.app-page').forEach(section => section.classList.toggle('active', section.id === page)); document.querySelectorAll('.nav-btn').forEach(button => button.classList.toggle('active', button.dataset.page === page)); }
  ...
  renderPurchaseTotal() { const today = inputDate(new Date()), spent = this.data.purchases.filter(row => row.date && row.date <= today), totalSpent = spent.reduce((sum, row) => sum + Number(row.amount || 0), 0), weeks = spent.length ? Math.max(1, Math.ceil((dateOnly(today) - dateOnly(spent[0].date)) / 604800000)) : 1; this.setValue('purchase-total', money(this.data.purchases.filter(row => Number(row.includeFlag)).reduce((sum, row) => sum + Number(row.amount || 0), 0))); this.setValue('purchase-spent', money(totalSpent)); this.setValue('average-spent', money(totalSpent / weeks)); }
